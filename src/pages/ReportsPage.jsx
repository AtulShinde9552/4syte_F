...
/**
 * Expands a sparse, unevenly-spaced chart series (e.g. [{name: "19 Aug", leads: 18}, ...])
 * into a continuous daily series with real timestamps, so the XAxis can use a
 * numeric time scale instead of a categorical one. Days with no data get `leads: null`
 * (paired with `connectNulls` on the <Area/>) so the line still reads as one continuous trend.
 */
function fillDailyGaps(rawChart) {
  if (!rawChart || rawChart.length === 0) return [];

  const currentYear = new Date().getFullYear();

  const parsed = rawChart
    .map((point) => ({
      date: new Date(`${point.name} ${currentYear}`),
      leads: point.leads,
    }))
...
      (p) => p.date.toDateString() === cursor.toDateString()
    );
    filled.push({
      date: cursor.getTime(),
      leads: match ? match.leads : null,
    });
...
/** Custom tooltip styled to match the card look from the Figma reference. */
...
        {leads != null ? `${leads.toLocaleString()} leads` : "No deliveries"}
      </div>
    </div>
  );
}
...
  // Gap-filled, time-scaled version of the raw chart data from the API
  const filledChartData = fillDailyGaps(chartData);
...
                  <AreaChart data={filledChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    <defs>
...
                      fill="url(#leadsFill)"
                      connectNulls
                      dot={{ r: 3, stroke: "#00A292", strokeWidth: 2, fill: "#fff" }}
                      activeDot={{ r: 5 }}
                    />
                  </AreaChart>
                </ResponsiveContainer>
